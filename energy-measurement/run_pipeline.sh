...
IMAGE_NAME="${IMAGE_NAME:-pytorch-lightning-medicao}"
MEDICAO_DIR="${MEDICAO_DIR:-$HOME/experimentos/medicao/repositorios/pytorch-lightning}"
RESULTS_DIR="${RESULTS_DIR:-$HOME/experimentos/medicao/resultados/pytorch-lightning/runs}"
LOGS_DIR="${LOGS_DIR:-$HOME/experimentos/medicao/resultados/pytorch-lightning/logs}"
...
STAGE_TIMEOUT_BUILD_DEFAULT=600
STAGE_TIMEOUT_TEST_DEFAULT=600
STAGE_TIMEOUT_TRAIN_DEFAULT=1100
...
ENERGY_STAGE_TIMEOUT_TRAIN_S="${ENERGY_STAGE_TIMEOUT_TRAIN_S:-$STAGE_TIMEOUT_TRAIN_DEFAULT}"
STAGE_TIMEOUT_EXIT=91
# Internal bridge with no external route: the DDP tests need an eth0 for GLOO_SOCKET_IFNAME.
NETWORK_NAME="${NETWORK_NAME:-energy-internal}"
NETWORK_CHECK_EXIT=92
# Empty on the campaign, which applies no CPU limit.
ENERGY_CPUSET="${ENERGY_CPUSET:-}"
TIME_FILE="/tmp/pytorch-lightning_time_$$.txt"
...
# Pre-registered exits and failed set per stage; a divergence does not change the recorded exit.
EXPECTED_EXIT_BUILD=0
EXPECTED_EXIT_TEST=1
EXPECTED_EXIT_TRAIN=0
TEST_EXPECTED_FAILED="helpers/test_datasets.py::test_mnist helpers/test_datasets.py::test_trial_mnist"
PIPELINE_EXIT=0
FAILED_STAGES=""
NONCONFORM_STAGES=""
...
if ! docker network inspect "$NETWORK_NAME" >/dev/null 2>&1; then
  ...
  echo "Docker network '$NETWORK_NAME' created (internal bridge)"
fi
...
# Hermeticity of the internal network, verified once per run before the baseline, outside the RAPL window.
# Conform when the container has eth0, no default route, and a connect by address fails by route.
network_check() {
  local verdict
  docker run --rm -i --network "$NETWORK_NAME" "$IMAGE_NAME" python3 - <<'PY' > "$NETWORK_FILE" 2>&1 || true
import errno, socket
  ...
  if [ "$verdict" != "conform" ]; then
    {
      ...
      echo "reason,internal network has an external route, no eth0, or the check did not run"
      cat "$NETWORK_FILE"
    } > "$NETWORK_DISCARD_FILE"
    echo "::error title=Network check::run $RUN_NUM: network '$NETWORK_NAME' failed the hermeticity check. Run not started; see $NETWORK_DISCARD_FILE"
...
# Conformity per stage: the expected exit and, for test and train, the failed set of junit.xml.
check_stage() {
  local stage="$1" stage_exit="$2" junit="$3"
  local expected verdict counts passed failed errors skipped xfailed failed_ids
  case "$stage" in
    build) expected="$EXPECTED_EXIT_BUILD" ;;
    test)  expected="$EXPECTED_EXIT_TEST" ;;
    train) expected="$EXPECTED_EXIT_TRAIN" ;;
  esac
  verdict="conform"
...
failed, xfailed, skipped, errors = [], 0, 0, 0
for c in cases:
    nodeid = c.get("classname", "").replace(".", "/") + ".py::" + c.get("name", "")
    if nodeid.startswith("tests/tests_pytorch/"):
        nodeid = nodeid[len("tests/tests_pytorch/"):]
    if c.find("failure") is not None:
        ...
        if c.find("skipped").get("type") == "pytest.xfail":
            xfailed += 1
        else:
            skipped += 1
passed = len(cases) - len(failed) - skipped - xfailed
print(passed, len(failed) - errors, errors, skipped, xfailed, " ".join(sorted(failed)))
PY
      ) || counts="- - - - - -"
    else
      counts="- - - - - -"
    fi
    read -r passed failed errors skipped xfailed failed_ids <<< "$counts"
    if [ "$stage" = "test" ]; then
      [ "$failed_ids" = "$TEST_EXPECTED_FAILED" ] || verdict="nonconform"
    else
      [ "$failed_ids" = "" ] && [ "$failed" = "0" ] || verdict="nonconform"
    fi
    {
      echo "${stage}_exit,$stage_exit"
      echo "${stage}_expected_exit,$expected"
      echo "${stage}_junit,$([ -f "$junit" ] && echo yes || echo no)"
      echo "${stage}_passed,$passed"
      echo "${stage}_failed,$failed"
      echo "${stage}_errors,$errors"
      echo "${stage}_skipped,$skipped"
      echo "${stage}_xfailed,$xfailed"
      echo "${stage}_failed_ids,${failed_ids:--}"
      echo "${stage}_verdict,$verdict"
    } >> "$ARTIFACT_FILE"
    echo "  $stage check: exit=$stage_exit passed=$passed failed=$failed errors=$errors skipped=$skipped xfailed=$xfailed -> $verdict"
  else
    {
      echo "${stage}_exit,$stage_exit"
      echo "${stage}_expected_exit,$expected"
    ...
    echo "  $stage check: exit=$stage_exit -> $verdict"
  fi
...
measure_stage() {
  ...
  case "$stage" in
    ...
    test)  stage_timeout="$ENERGY_STAGE_TIMEOUT_TEST_S" ;;
    train) stage_timeout="$ENERGY_STAGE_TIMEOUT_TRAIN_S" ;;
  esac
  cname="pytorch-lightning-run${RUN_ID}-${stage}"
  ...
  # GITHUB_RUN_ID is the run number, as in the previous campaign; it seeds the test order.
  # fd3 keeps the workload stderr while `time` measures inside the container.
  # junit.xml leaves the container after the timed block, for the conformity check.
  /usr/bin/time -f "%e" -o "$TIME_FILE" \
    ...
    docker run --rm --name "$cname" --privileged --network "$NETWORK_NAME" \
      ...
      "$IMAGE_NAME" \
      bash -c 'exec 3>&2; TIMEFORMAT="%R %U %S"; { time bash /medicao/commands.sh "$STAGE" 2>&3; } 2>/timing/time.txt; rc=$?; cp -f /project/tests/tests_pytorch/junit.xml /timing/junit.xml 2>/dev/null; exit $rc' \
      2>&1 | tee "$stage_log"
  ...
  local junit_file="$RESULTS_DIR/junit_run_${RUN_ID}_${stage}.xml"
  rm -f "$junit_file"
  if [ -f "$timing_dir/junit.xml" ]; then
    cp -f "$timing_dir/junit.xml" "$junit_file"
  fi
  check_stage "$stage" "$stage_exit" "$junit_file"
  CURRENT_CONTAINER=""
...
# The job's order: install, then the tests; train is the pre-registered cut of the same suite.
measure_stage build
measure_stage test
measure_stage train
